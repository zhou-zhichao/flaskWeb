...
def data_elem_align(file_path, elem_standard, sheet_name):
    ...
    # df1.rename(columns={'表/视图名称': '原始表', '标准表': '表/视图名称', '字段': '字段名'}, inplace=True)
...
def statistics(version):
    try:
        ...
        # df['上级编码'] = df['上级编码'].replace(np.nan, '')
        business_demand['业务线确认'] = business_demand['业务线确认'].replace(np.nan, '')
        ...
        data = {
            'B2': first,
            'B3': second,
            'B4': standard_master_num,
            'B5': business_master_num,
            'B6': standard_business_unit_num,
            'B7': app_num,
            'B8': standard_business_unit_num - app_num,
            'B10': overlapping_num,
            ...
            'B16': business_cover_ratio,
            'B17': implementation_ratio,
            'B19': out_demand_cover_ratio
        }
...
def xlsx_func(filename):
    ...
    if field == "外供数据检查确认":
        ...
        status = data_elem_align(f"file/temp/{version}_外供数据检查确认.xlsx", "file/standard/" +
        ...
        # version = filename.rsplit(os.path.sep)[-1].rsplit("_", 1)[0]
        check_and_create_folder("file/modify/")
        status = first_sheet_write(filename, os.path.join('file', 'temp', f'{version}_对外数据要求确认.xlsx'))
        if status:
            return status
        shutil.copy(filename, f'file/temp/{version}_对外数据要求确认.xlsx')
        # status = data_elem_align(f'file/temp/{version}_对外数据要求确认.xlsx',
        #                          "file/standard/" + version.split("_")[0] + "_业务线数据元素映射.xlsx",
        #                          "对外数据要求检查")
        # if status:
        #     return status
...
def filter_xlsx(list_of_paths):
    ...
    return list(
        filter(lambda path: isinstance(path, str) and (path.endswith(".xlsx") or path.endswith('.zip')), list_of_paths))
